...
export default function DashboardPage() {
    ...
    const calculateStats = (allOrders: Order[]) => {
        ...
        const completedOrders = allOrders.filter(o => o && (o.status === 'delivered' || o.status === 'completed')).length;
        ...
        setStats([
            ...
            { label: 'Completed Orders', value: completedOrders.toString(), detail: `${totalOrders > 0 ? Math.floor((completedOrders / totalOrders) * 100) : 0}% of total`, isPositive: true, icon: Package },
        ]);
    };
    ...
    const trendData = useMemo(() => {
        ...
        if (timeRange === 'Today' || timeRange === 'Custom') {
            ...
            filteredOrdersByRange.forEach(o => {
                ...
                if (week < 4) points[week] += (o.total || 0);
            });
        }
                ...
                <div className={styles.chartCard}>
                    ...
                    <div className={styles.itemsList}>
                        {filteredOrdersByRange.length > 0 ? (
                            Object.entries(filteredOrdersByRange
                                ...
                                .map(([name, sales]: any, i) => (
                                    <div key={i} className={styles.topItemRow}>
                                        ...
                                        <div className={styles.itemRevenue}>
                                            <span>₦{((orders.flatMap(o => Array.isArray(o.items) ? o.items : []).filter(item => item && item.name).find(it => it.name === name)?.price || 0) * sales).toLocaleString()}</span>
